...
export class CommonService {
    async paginate<T extends BaseModel>(
        ...
        const lastItem: T = results.length > 0 && results.length === paginationDto.take ? results[results.length - 1] : null;
        return {
            items: results,
            metaData: {
    ...
    composeFindManyOptions<T>(
        ...
        for (const [key, value] of Object.entries(paginationDto)) {
            ...
            if ((splitKey.length === 2 || splitKey.length === 3) && !overrideFindOptions[splitKey[0]]) {
